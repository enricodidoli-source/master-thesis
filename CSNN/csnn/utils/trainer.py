...
# Density Based initializatioin (initial definition of scores)
def dd_initialize(dataset_train, dataset_valid, alpha=0.75, max_iter=int(5),#max_iter=int(1e6),
                  tol=1e-7, dd_sample_size=100, dd_threshold=95,
                  return_scores=False, bayesian_dd=False, n_restarts=5):
    """ computed the scores using KDE, since score initialization is density based"""
    ...
    if len(allpos_sample) == 0:
        print('Try chencking in DensityDifference .fit -> len(2*sample_size).'
                'if too high, probably the problem is in dd_sample_size in tuning_ball_logistic')

    # computes the scores based on the fitted KDE
    if return_scores:
...
def train_initialize(dataset_train, dataset_valid, diff, allpos_sample, 
                     allneg_sample, alpha=0.75, max_iter=int(5), #max_iter=int(1e6), 
                     tol=1e-7, 
    ...
    for i in range(n_restarts):
        ...
        accs.append(tr_acc)
        models.append(model.cpu()) # parameters?
...
def train_initialize_loaded(Xinit_train, yinit_train,
                     alpha=0.75, max_iter=int(5), #max_iter=int(1e6), 
                     tol=1e-7,
                     dd_sample_size=10000, dd_threshold=95,
                     lr=1e-5, architecture=[8,4,2,1], negative_penalty=1,
                     bayesian_dd=False, n_restarts=5):
...
def train_head(dataset_train, dataset_valid, model, alpha=0.75, 
               max_iter=int(5), #max_iter=int(1e6), 
               tol=1e-7, dd_sample_size=10000, 
               dd_threshold=95, 
               lr=1e-5, architecture=[8,4,2,1], negative_penalty=1,
               bayesian_dd=False, n_restarts=5):
    model.to(device)
...
def finetune(dataset_train, dataset_valid, model, alpha=0.75, max_iter=int(5), #max_iter=int(1e6),
