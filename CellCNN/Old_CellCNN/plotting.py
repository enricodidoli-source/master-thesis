...
def plot_results(results, samples, phenotypes, labels, outdir,
                 ...
                 regression=False, show_filters=True, show_tsne = True, plot_test = False):
    ...
    if plot_test:
        ...
        samples = [samples[0]]
        samples_for_true_cells = samples.copy()
        labels_column = samples_for_true_cells[0]['IsBlast']
        
    samples = [sample.drop(['IsBlast'], axis=1) for sample in samples]

    col_names = samples[0].columns.to_list()
    ...
    if plot_test:
        lables_column = samples_for_true_cells[0].pop('IsBlast') 
        true_cells_no_labels = samples_for_true_cells[0]
    if results['scaler'] is not None:
        print(f'scaler present')
        x = results['scaler'].transform(x)
        
        if plot_test:
            true_cells = results['scaler'].transform(true_cells_no_labels)
            true_cells = pd.DataFrame(true_cells)
            true_cells['IsBlast'] = labels_column
    ...
    if plot_test:
        x_for_tsne_true_cells = true_cells.iloc[tsne_idx].copy()
        tsne_labels_column = x_for_tsne_true_cells.pop('IsBlast').to_numpy()
    ...
    if show_tsne:
        if plot_test:
            print('Red-grey tsne started')
            #show grey-red tsne plot
            colors = np.where(tsne_labels_column == 1, 'red', 'gray')
            print(f'colors: {colors}')
            fig, ax = plt.subplots(figsize=(6, 5))
            ax.scatter(x_tsne[:, 0], x_tsne[:, 1], 
                       s=0.1, 
                       marker='o', 
                       c=colors,              # <-- Usiamo l'array di colori direttamente
                       alpha=0.5, 
                       edgecolors='face')  
            
            fig_path = os.path.join(outdir, '12.0.True_cells_tsne')
        
            # Salva la figura
            plt.savefig(fig_path, format='pdf', bbox_inches='tight')
            print(f"Salvato in: {fig_path}")
        
        print('tsne by filter started')
        
        fig_path = os.path.join(outdir, '12.tsne_all_cells')
        plot_tsne_grid(x_tsne, x_for_tsne, fig_path, labels=labels, fig_size=(40, 10), #(20,20)
                       point_size=5)
    ...
    for i_filter in keep_idx:
        ...
        print(f'plot_tsne_selection_grid started')
        ...
        print(f'plot_selected_subset started')
...
def plot_filters(results, labels, outdir):
    """Shows the most acrivated cells by the convolutional filters"""
    mkdir_p(outdir)
    nmark = len(labels)
    
    # plot the filter weights of the best network
    w_best = results['w_best_net'] # best ceonvolutional filters across the tested
    
    idx_except_bias = np.hstack([np.arange(0, nmark), np.arange(nmark + 1, w_best.shape[1])])  # generate the range of indexes to mantain
    nc = w_best.shape[1] - (nmark + 1)  
    labels_except_bias = labels + [f"out {i}" for i in range(nc)] # remove the bias
    
    w_best = w_best[:, idx_except_bias] # remove the bias
    fig_path = os.path.join(outdir, '6.best_net_weights.pdf')
    plot_nn_weights(w_best, labels_except_bias, fig_path, fig_size=(10, 10)) # shows 
    
    # plot the filter clustering
    cl = results['clustering_result']  
    cl_w = cl['w'][:, idx_except_bias]  #cluster weigths
...
def plot_nn_weights(w, x_labels, fig_path, row_linkage=None, y_labels=None, fig_size=(10, 3)):
    ...
    if w.shape[0] > 1: #if there are more than one filter
        plt.figure(figsize=fig_size)

        # shows the heatmap of the filters, clustered by cosine similarity
    ...
    else:  #otherwise no clustering is needed and  just shows aan heatmap
...
def plot_marker_distribution(datalist, namelist, labels, grid_size, fig_path=None, letter_size=16,
                             figsize=(9, 9), ks_list=None, colors=None, hist=False, max_cells=10000):
    ...
    for x in datalist:
        if len(x) > max_cells:
            ...
            datalist_plot.append(x[idx])
        else:
            datalist_plot.append(x)
    datalist = datalist_plot
...
def plot_selected_subset(xc, zc, x, labels, sample_sizes, phenotypes, outdir, suffix,
                              ...
                              regression=False, selected_freq = False):
    print('here')
    ks_values = []
    nmark = x.shape[1]
    ...
    if selected_freq:
        ...
        if regression:
            ...
            print(f'phenotypes: {np.unique(phenotypes)}')
            print(f'all phenotypes', phenotypes)
            ...
            print("frequencies keys:", list(frequencies.keys()))
            print("frequencies values:", frequencies)
            print("n_pheno:", n_pheno)
            print("phenotypes unique:", np.unique(phenotypes))
...
def plot_tsne_grid(z, x, fig_path, labels=None, fig_size=(9, 9), g_j=4,
    ...
    print(x)
    print(z)
    print(np.issubdtype(z.dtype, np.number), f'z deve essere numerico. type found: {z.dtype}\n')
    print(np.issubdtype(x.dtype, np.number), f'x deve essere numerico. type found: {x.dtype}\n')  
    if labels is None:
    ...
    for i, seq_index in enumerate(range(ncol)):
        ...
        col = seq_index
        # Create scatter plot
        im = ax.scatter(z[:, 0], z[:, 1], s=point_size, marker='o', c=x[:, col],
...
def plot_tsne_selection_grid(z_pos, x_pos, z_neg, vmin, vmax, fig_path,
                             labels=None, fig_size=(9, 9), g_j=4, s=.5, suffix='png'):
    ncol = x_pos.shape[1]
    ...
    fig_size = (g_j * single_w, g_i * single_h)
...
def plot_only_tsne_heat(results, samples, phenotypes, labels, outdir,
                 filter_diff_thres=.2, filter_response_thres=0, response_grad_cutoff=None,
                 stat_test=None, log_yscale=False,
                 group_a='group A', group_b='group B', group_names=None, tsne_ncell=10000,
    ...
    fig_path = os.path.join(outdir, '20.tsne_all_cells')
    plot_tsne_grid(x_tsne, x_for_tsne, fig_path, labels=labels, fig_size=(40, 10), #(20,20)
                   point_size=5)
    ...
    # filters analysis 
    return_filters = []
    for i, i_filter in enumerate(keep_idx):
      #if i == 0:  
        w = filters[i_filter, :nmark] # extract filters' weights 
        b = filters[i_filter, nmark] #extract filters' biases
        g = np.sum(w.reshape(1, -1) * x, axis=1) + b  # RELU activation. computes responses
...
def plot_only_filters(results, samples, phenotypes, labels, outdir,
                 filter_diff_thres=.2, filter_response_thres=0, response_grad_cutoff=None,
                 stat_test=None, log_yscale=False,
                 group_a='group A', group_b='group B', group_names=None, tsne_ncell=10000,
                 regression=False, show_filters=True):
