...
from tensorflow.keras import layers, initializers, regularizers, optimizers, callbacks, metrics
...
def train_model(train_samples, train_phenotypes, outdir,
    ...
    if scale:
        X_train, z_scaler = data_transformation(X_train, labels = labels)  # apply scaling
    else:
        z_scaler = None
    ...
    for irun in range(nrun):
        ...
        model = build_model(ncell, nmark, nfilter,
                            coeff_l1, coeff_l2, k,
                            dropout, dropout_p, regression, n_classes, lr) #, run_seed)
        ...
        for _ in range(1):
            ...
            if not regression:
                ...
                model.fit(X_tr, y_tr,
                          epochs=max_epochs, batch_size=bs, callbacks=[check, earlyStopping],
                          validation_data=(X_v, y_v), verbose=0)# verbose=verbose)
            ...
            if not regression:
                valid_metric = model.evaluate(X_v, y_v, verbose=verbose)[-1] # test the model on the validation set
...
def build_model(ncell, nmark, nfilter, coeff_l1, coeff_l2,
                k, dropout, dropout_p, regression, n_classes, lr=0.01, init_seed=42):
