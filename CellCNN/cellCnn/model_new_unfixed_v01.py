...
import random
from cellCnn.utils_new_unfixed import combine_samples, normalize_outliers_to_control
from cellCnn.utils_new_unfixed import cluster_profiles, keras_param_vector
from cellCnn.utils_new_unfixed import generate_subsets, generate_biased_subsets
from cellCnn.utils_new_unfixed import get_filters_classification, get_filters_regression
from cellCnn.utils_new_unfixed import mkdir_p
...
def extract_cells_labels(dataset):
    cells_labels = []
    dataset_without_labels = []
    for subset in dataset:
        subset_cells_labels = subset['IsBlast'].values
        cells_labels.append(subset_cells_labels)
        
        subset_without_labels = subset.drop(columns = ['IsBlast'])
        dataset_without_labels.append(subset_without_labels.values)
